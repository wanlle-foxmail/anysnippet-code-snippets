...
SQL_PATH = SNIPPET_ROOT / "src" / "find_duplicate_values.sql"
...
class MySQLSnippetTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.temp_dir = tempfile.TemporaryDirectory(prefix="find-duplicate-mysql-")
...
class FindDuplicateValuesWithSqlTests(MySQLSnippetTestCase):
    def setUp(self):
        self.run_mysql(
            """
    ...
    def insert_users(self, rows):
        values = ",\n".join(f"({user_id}, {email_value})" for user_id, email_value in rows)
        self.run_mysql(
            f"""
            USE snippet_test;
    ...
    def test_returns_duplicate_values_with_their_counts(self):
        self.insert_users(
            [
                (1, "'alpha@example.com'"),
                (2, "'alpha@example.com'"),
        ...
        self.assertEqual(
            [("beta@example.com", "3"), ("alpha@example.com", "2")],
            self.fetch_rows(),
        )

    def test_excludes_unique_values_from_the_result(self):
        self.insert_users(
            [
                (1, "'alpha@example.com'"),
                (2, "'alpha@example.com'"),
        ...
        self.assertEqual([("alpha@example.com", "2")], self.fetch_rows())

    def test_ignores_null_values_before_grouping(self):
        self.insert_users(
            [
                (1, "'alpha@example.com'"),
                (2, "'alpha@example.com'"),
    ...
    def test_orders_equal_duplicate_counts_by_email(self):
        self.insert_users(
            [
                (1, "'alpha@example.com'"),
                (2, "'alpha@example.com'"),
        ...
        self.assertEqual(
            [("alpha@example.com", "2"), ("beta@example.com", "2")],
            self.fetch_rows(),
        )

    def test_returns_empty_result_when_every_value_is_unique(self):
        self.insert_users(
            [
                (1, "'alpha@example.com'"),
                (2, "'beta@example.com'"),
